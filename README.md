...
EHR code lists that were previously published are listed here with references:
...
PLINK commands to extract whole exome sequencing (WES) data for UK Biobank (on DNAnexus):

Equivalent commands were used for Genes & Health (on TRE).

The below R scripts are split in data preparation (**prep_\*.R**) and analysis (**analysis_*.R**).
