...
# Read in genetic-determined ancestry
system("dx download /ID_lists/Genetic/*")
eurids <- read.table("EUR_IDs.txt", header = TRUE, stringsAsFactors = FALSE)
afrids <- read.table("AFR_IDs.txt", header = TRUE, stringsAsFactors = FALSE)
sasids <- read.table("SAS_IDs.txt", header = TRUE, stringsAsFactors = FALSE)
easids <- read.table("EAS_2550_subject_list_103356.txt", header = TRUE, stringsAsFactors = FALSE)
amrids <- read.table("AMR_470_subject_list_103356.txt", header = TRUE, stringsAsFactors = FALSE)

alldata$genetic_ancestry <- NA
alldata[alldata$Participant.ID %in% eurids$FID,]$genetic_ancestry <- "EUR"
alldata[alldata$Participant.ID %in% afrids$FID,]$genetic_ancestry <- "AFR"
alldata[alldata$Participant.ID %in% sasids$FID,]$genetic_ancestry <- "SAS"
alldata[alldata$Participant.ID %in% easids$FID,]$genetic_ancestry <- "EAS"
alldata[alldata$Participant.ID %in% amrids$FID,]$genetic_ancestry <- "AMR"
...
# Identify all IDs with haemolytic anaemia diagnosis in GP/HES data
haemcodes <- NULL
for (code in c("read_2", "read_3")) {
  haemdata <- read.csv(paste0(code, "_haemolytic_anaemia.txt"), header = TRUE, stringsAsFactors = FALSE, sep = "\t")
  haemcodes <- unique(c(haemcodes, haemdata[,1]))
}
haemids <- read_GP(haemcodes)

icd10codes <- read.csv("ICD10_haemolytic_anaemia.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
icd10codes <- unique(icd10codes$ICD10)
icd9codes <- read.csv("ICD9_haemolytic_anaemia.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
icd9codes <- unique(icd9codes$Code)

icd10data <- read_ICD10(icd10codes)
icd9data <- read_ICD9(icd9codes)
haemids <- unique(c(haemids, icd10data$eid, icd9data$eid))

# Generate marker variables of G6PD deficiency and/or haemolytic anaemia diagnoses
alldata$g6pd <- 0
alldata[alldata$Participant.ID %in% g6pdids,]$g6pd <- 1
alldata$haem <- 0
alldata[alldata$Participant.ID %in% haemids,]$haem <- 1

alldata$g6pd_or_haem <- 0
alldata[alldata$g6pd == 1 | alldata$haem == 1,]$g6pd_or_haem <- 1
alldata$g6pd_and_haem <- 0
alldata[alldata$g6pd == 1 & alldata$haem == 1,]$g6pd_and_haem <- 1
...
for (code in c("read_2", "read_3")) {
  ...
  readcodes <- read.table(paste0(code, "_diagnosis.txt"), header = TRUE, stringsAsFactors = FALSE, sep = "\t")
...
drugcodes <- read.table("read_2_drugs.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
icddrugcodes <- read.table("ICD10_drugs.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
...
katie_excl2 <- read.csv("read_2_exclusion.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
katie_excl3 <- read.csv("read_3_exclusion.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
katie_excl <- unique(c(katie_excl2$Code, katie_excl3$Code))

# Read in ICD-10 non-T1D/T2D exclusion codes
exclcodes <- read.csv("ICD10_exclusion.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
...
### Type 2 diabetes prevalent at baseline assessment centre visit ###

alldata$t2d_anyage_0 <- alldata$t2d_anyage
alldata[alldata$t2d_before_centre == 0,]$t2d_anyage_0 <- 0

alldata$t2d_over35_0 <- alldata$t2d_over35
alldata[alldata$t2d_before_centre == 0,]$t2d_over35_0 <- 0

### SENSITIVITY ANALYSIS PHENOTYPES - needed for exclusion if prevalent at baseline assessment centre visit ###

### End-stage renal disease diagnoses (or eGFR < 15mL/min) ###

# Identify IDs with diagnosis of end-stage renal disease in GP/HES data and first occurrence of record
system("dx download /EHR_codes/End_stage_renal_disease/*")
readcodes <- read.csv("MULTIPLY_Read_ESRD.csv", header = TRUE, stringsAsFactors = FALSE)
icd10codes <- read.csv("MULTIPLY_ICD10_ESRD.csv", header = TRUE, stringsAsFactors = FALSE)
esrddata <- first_occurence(ICD10 = unique(icd10codes$ICD10code), GP = unique(readcodes$ReadCode), OPCS = "", cancer = "")
colnames(esrddata)[colnames(esrddata) == "date"] <- "esrd_date_EHR"
alldata <- merge(alldata, esrddata[names(esrddata) != "source"], by.x = "Participant.ID", by.y = "eid", all.x = TRUE)
alldata <- ukb_dates(date_variable = "esrd_date_EHR")

# Identify earliest date of ESRD recorded from self-reported and EHR data
alldata <- ukb_dates(date_variable = "Date.of.end.stage.renal.disease.report")
alldata$esrd_date <- apply(alldata[c("esrd_date_EHR", "Date.of.end.stage.renal.disease.report")], 1, FUN = my_min)

colnames(alldata)[colnames(alldata) == "Creatinine...Instance.0"] <- "creatinine"
colnames(alldata)[colnames(alldata) == "Cystatin.C...Instance.0"] <- "cystatin"

# Convert creatinine to required units (umol/L -> mg/dL) (using conversion from Nephrol Dial Transplant (2004) 19 [Suppl 2]: ii42–ii43)
alldata$creatinine <- 0.0113*alldata$creatinine

# Calculate eGFR using creatinine- or cystatin C-only equations if only one and not both measures are present at baseline visit
alldata$eGFR_method <- NA
alldata[!is.na(alldata$creatinine) & !is.na(alldata$cystatin),]$eGFR_method <- "Both"
alldata[!is.na(alldata$creatinine) & is.na(alldata$cystatin),]$eGFR_method <- "Creatinine"
alldata[!is.na(alldata$cystatin) & is.na(alldata$creatinine),]$eGFR_method <- "Cystatin"

# Calculate eGFR using 2021 CKD-EPI Creatinine-Cystatin C Equation (Inker, 2021. NEJM) where both measures are present
alldata$eGFR <- NA
alldata[alldata$Sex == "Male",]$eGFR <- 135*(pmin(alldata[alldata$Sex == "Male",]$creatinine/0.9, 1)^(-0.144))*(pmax(alldata[alldata$Sex == "Male",]$creatinine/0.9, 1)^(-0.544))*
  (pmin(alldata[alldata$Sex == "Male",]$cystatin/0.8, 1)^(-0.323))*(pmax(alldata[alldata$Sex == "Male",]$cystatin/0.8, 1)^(-0.778))*(0.9961^alldata[alldata$Sex == "Male",]$age)
alldata[alldata$Sex == "Female",]$eGFR <- 135*(pmin(alldata[alldata$Sex == "Female",]$creatinine/0.7, 1)^(-0.219))*(pmax(alldata[alldata$Sex == "Female",]$creatinine/0.7, 1)^(-0.544))*
  (pmin(alldata[alldata$Sex == "Female",]$cystatin/0.8, 1)^(-0.323))*(pmax(alldata[alldata$Sex == "Female",]$cystatin/0.8, 1)^(-0.778))*(0.9961^alldata[alldata$Sex == "Female",]$age)*0.963

# Calculate eGFR using creatinine-only 2021 CKD-EPI Creatinine Equation (Inker, 2021. NEJM) where cystatin C is not present
alldata[is.na(alldata$cystatin) & alldata$Sex == "Male",]$eGFR <- 142*(pmin(alldata[is.na(alldata$cystatin) & alldata$Sex == "Male",]$creatinine/0.9, 1)^(-0.302))*(pmax(alldata[is.na(alldata$cystatin) & alldata$Sex == "Male",]$creatinine/0.9, 1)^(-1.2))*
  (0.9938^alldata[is.na(alldata$cystatin) & alldata$Sex == "Male",]$age)
alldata[is.na(alldata$cystatin) & alldata$Sex == "Female",]$eGFR <- 142*(pmin(alldata[is.na(alldata$cystatin) & alldata$Sex == "Female",]$creatinine/0.7, 1)^(-0.241))*(pmax(alldata[is.na(alldata$cystatin) & alldata$Sex == "Female",]$creatinine/0.7, 1)^(-1.2))*
  (0.9938^alldata[is.na(alldata$cystatin) & alldata$Sex == "Female",]$age)*1.012

# Calculate eGFR using cystatin C-only 2012 CKD-EPI Cystatin C Equation (Inker, 2012. NEJM) where creatinine is not present
alldata[is.na(alldata$creatinine) & alldata$Sex == "Male",]$eGFR <- 133*(pmin(alldata[is.na(alldata$creatinine) & alldata$Sex == "Male",]$cystatin/0.8, 1)^(-0.499))*(pmax(alldata[is.na(alldata$creatinine) & alldata$Sex == "Male",]$cystatin/0.8, 1)^(-1.328))*
  (0.996^alldata[is.na(alldata$creatinine) & alldata$Sex == "Male",]$age)
alldata[is.na(alldata$creatinine) & alldata$Sex == "Female",]$eGFR <- 133*(pmin(alldata[is.na(alldata$creatinine) & alldata$Sex == "Female",]$cystatin/0.8, 1)^(-0.499))*(pmax(alldata[is.na(alldata$creatinine) & alldata$Sex == "Female",]$cystatin/0.8, 1)^(-1.328))*
  (0.996^alldata[is.na(alldata$creatinine) & alldata$Sex == "Female",]$age)*0.932

# Assign IDs binary ESRD variable marker if ESRD diagnosis prior to baseline assessment centre or eGFR < 15mL/min
alldata$esrd <- 0
alldata[(!is.na(alldata$esrd_date) & !is.na(alldata$date_centre_0) & alldata$esrd_date <= alldata$date_centre_0) | (!is.na(alldata$eGFR) & alldata$eGFR < 15),]$esrd <- 1

### HIV infection diagnoses ###

system("dx download /EHR_codes/HIV/*")
readcodes <- read.csv("MULTIPLY_Read_hiv.csv", header = TRUE, stringsAsFactors = FALSE)
icd10codes <- read.csv("MULTIPLY_ICD10_hiv.csv", header = TRUE, stringsAsFactors = FALSE)
hivdata <- first_occurence(ICD10 = unique(icd10codes$ICD10code), GP = unique(readcodes$ReadCode), OPCS = "", cancer = "")
colnames(hivdata)[colnames(hivdata) == "date"] <- "hiv_date_EHR"
alldata <- merge(alldata, hivdata[names(hivdata) != "source"], by.x = "Participant.ID", by.y = "eid", all.x = TRUE)
alldata <- ukb_dates(date_variable = "hiv_date_EHR")

# Identify those who self-reported having HIV
# & extract self-reported age that HIV was diagnosed from corresponding array
alldata$hiv_self <- 0
index <- as.numeric(rownames(noncan0[apply(noncan0, 1, function(x) "hiv/aids" %in% x),]))
alldata$hiv_self[index] <- 1

# Identify the array/order that HIV was reported - to extract corresponding age
alldata <- noncancer_age(disease_codes = "hiv/aids", condition_name = "hiv")

# Estimate date of self-diagnosis from age of self-diagnosis
alldata$hiv_date_self <- as.Date(alldata$dob %m+% years(floor(alldata$hiv_age_self)))

# Generate earliest date of HIV diagnosis from EHR and self-reported records
alldata$hiv_date <- apply(alldata[c("hiv_date_EHR", "hiv_date_self")], 1, FUN = my_min)

# Assign IDs binary HIV variable marker if HIV diagnosis prior to baseline assessment centre (or self-reported)
alldata$hiv <- 0
alldata[(alldata$hiv_self == 1) |
          (!is.na(alldata$HIV.1.seropositivity.for.Human.Immunodeficiency.Virus...Instance.0) & alldata$HIV.1.seropositivity.for.Human.Immunodeficiency.Virus...Instance.0 == "True") |
          (!is.na(alldata$hiv_date) & !is.na(alldata$date_centre_0) & (alldata$hiv_date <= alldata$date_centre_0)),]$hiv <- 1

### Iron-deficiency anaemia diagnoses ###

system("dx download /EHR_codes/Iron_deficiency_anaemia/*")
readcodes <- read.csv("MULTIPLY_Read_iron_deficiency_anaemia.csv", header = TRUE, stringsAsFactors = FALSE)
icd10codes <- read.csv("MULTIPLY_ICD10_iron_deficiency_anaemia.csv", header = TRUE, stringsAsFactors = FALSE)
irondata <- first_occurence(ICD10 = unique(icd10codes$ICD10code), GP = unique(readcodes$ReadCode), OPCS = "", cancer = "")
colnames(irondata)[colnames(irondata) == "date"] <- "iron_def_anaem_date_EHR"
alldata <- merge(alldata, irondata[names(irondata) != "source"], by.x = "Participant.ID", by.y = "eid", all.x = TRUE)
alldata <- ukb_dates(date_variable = "iron_def_anaem_date_EHR")

# Identify those who self-reported having iron-deficiency anaemia
# & extract self-reported age that iron-deficiency anaemia was diagnosed from corresponding array
alldata$iron_def_anaem_self <- 0
index <- as.numeric(rownames(noncan0[apply(noncan0, 1, function(x) "iron deficieny anaemia" %in% x),]))
alldata$iron_def_anaem_self[index] <- 1

# Identify the array/order that iron-deficiency anaemia was reported - to extract corresponding age
alldata <- noncancer_age(disease_codes = "iron deficiency anaemia", condition_name = "iron_def_anaem")

# Estimate date of self-diagnosis from age of self-diagnosis
alldata$iron_def_anaem_date_self <- as.Date(alldata$dob %m+% years(floor(alldata$iron_def_anaem_age_self)))

# Generate earliest date of iron-deficiency anaemia diagnosis from EHR and self-reported records
alldata$iron_def_anaem_date <- apply(alldata[c("iron_def_anaem_date_EHR", "iron_def_anaem_date_self")], 1, FUN = my_min)

# Assign IDs binary iron-deficiency anaemia variable marker if diagnosis prior to baseline assessment centre (or self-reported)
alldata$iron_def_anaem <- 0
alldata[alldata$iron_def_anaem_self == 1 |
          (!is.na(alldata$iron_def_anaem_date) & !is.na(alldata$date_centre_0) & alldata$iron_def_anaem_date < alldata$date_centre_0),]$iron_def_anaem <- 1

### Splenectomy recordings ###

system("dx download /EHR_codes/Splenectomy/*")
icd10codes <- read.csv("ICD10_splenectomy.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
readcodes <- read.csv("read_codes_splenectomy.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
opcscodes <- read.csv("OPCS_splenectomy.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")

spleendata <- first_occurence(ICD10 = icd10codes[,1], GP = readcodes[,1], OPCS = opcscodes[,1], cancer = "")
colnames(spleendata)[colnames(spleendata) == "date"] <- "splenectomy_date"
alldata <- merge(alldata, spleendata[names(spleendata) != "source"], by.x = "Participant.ID", by.y = "eid", all.x = TRUE)
alldata <- ukb_dates(date_variable = "splenectomy_date")

# Assign IDs binary splenectomy variable marker if surgery performed prior to baseline assessment centre
alldata$splenectomy <- 0
if (nrow(alldata[!is.na(alldata$splenectomy_date) & !is.na(alldata$date_centre_0) & alldata$splenectomy_date < alldata$date_centre_0,]) > 0) {
  alldata[!is.na(alldata$splenectomy_date) & !is.na(alldata$date_centre_0) & alldata$splenectomy_date < alldata$date_centre_0,]$splenectomy <- 1
}

### Recent blood transfusions (<3 months) ###

system("dx download /EHR_codes/Blood_transfusion/*")
blooddata <- NULL
for (code in c("read_2", "read_3")) {
  readcodes <- read.csv(paste0("blood_transfusion_", code, "_codes.csv"), header = TRUE, stringsAsFactors = FALSE)
  readdata <- read_GP(unique(readcodes[,1]))
  blooddata <- rbind(blooddata, readdata)
}
icd10codes <- read.csv("ICD10_blood_transfusion.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
icd10data <- read_ICD10(icd10codes[,1])
opcscodes <- read.csv("OPCS_blood_transfusion.txt", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
opcsdata <- read_OPCS(opcscodes[,1])

# Bind HES and GP blood transfusion records with dates together
blooddata$transfusion_date <- blooddata$event_dt
icd10data$transfusion_date <- icd10data$epiend
opcsdata$transfusion_date <- opcsdata$opdate

blooddata <- rbind(blooddata[c("eid", "transfusion_date")], icd10data[c("eid", "transfusion_date")], opcsdata[c("eid", "transfusion_date")])

# Estimate date of 3-months after blood transfusion - when HbA1c becomes reliable again
blooddata$transfusion_date_3mths <- as.Date(blooddata$transfusion_date) %m+% months(3)
alldata <- merge(alldata, blooddata[c("eid", "transfusion_date", "transfusion_date_3mths")], by.x = "Participant.ID", by.y = "eid", all.x = TRUE)
alldata <- ukb_dates(date_variable = c("transfusion_date", "transfusion_date_3mths"))

# Assign IDs binary blood transfusion variable marker if received recently prior to baseline assessment centre
alldata$blood_transfusion <- 0
alldata[(!is.na(alldata$date_centre_0) & !is.na(alldata$transfusion_date) & !is.na(alldata$transfusion_date_3mths) & (alldata$date_centre_0 >= alldata$transfusion_date) & (alldata$date_centre_0 < alldata$transfusion_date_3mths)),]$blood_transfusion <- 1

# Remove columns relating to blood transfusion dates so can have one row per ID
alldata <- alldata[, -which(names(alldata) %in% c("transfusion_date", "transfusion_date_3mths"))]
alldata <- alldata[!duplicated(alldata$Participant.ID),]
...
alldata <- alldata[c("Participant.ID", "Sex", "ethnicity", "ethnic_group", "sub_ethnic_group", "genetic_ancestry", "HbA1c", "Glucose", "Fasting", "BMI", "centre", "age",
                     "g6pd", "haem", "g6pd_or_haem", "g6pd_and_haem", "diabetes_loose", "pregnant",
                     ...
                     "esrd", "hiv", "iron_def_anaem", "splenectomy", "blood_transfusion")]
